export default function TaskList({
  ...
  <div className="row g-3">
    ...
    <div className="d-none d-md-block col-md-4 col-lg-2">
      ...
      >
        {tasks.length === 0 ? (
          <div className=" text-center">Mis tareas</div>
        ) : (
    ...
    <div className="col-12 col-md-8 col-lg-10">
      ...
      >
        {tasks.length === 0 ? (
          <div className="p-5 text-center text-muted">No tienes tareas creadas</div>
        ) : (
          sortedTasks.map((task, index) => (
            <div className="border-bottom py-2" key={task.id}>
              <span className="text-muted small" id={`list-item-${task.id}`}>
                Tarea {index + 1}
              </span>
              <div className="d-flex align-items-center flex-wrap gap-2">
                <strong className="flex-grow-1 text-truncate" style={{ minWidth: "100px" }}>
                  {task.title}
                </strong>
                <span className={`badge ${priorityColors[task.priority] || "bg-light"}`}>
                  {task.priority}
                </span>
